...
module.exports = function(app, options) {
  ...
  if (options.loginPath !== false) {
    ...
    app.post(options.loginPath || '/login', (req, res, next) => {
      const isGeero = req.hostname === 'geero.webduino.com.cn';

      passport.authenticate('loopback-oauth2-local', (err, user, info) => {
        if (err) {
          return next(err);
        }
        if (!user) {
          res.redirect(options.loginPage + '?loginFailed' || '/login')
        }
        req.login(user, function (err) {
          if (err) { return next(err); }
          if (isGeero) {
            res.cookie('geero', true, {
              domain: 'webduino.com.cn'
            });
            return res.redirect('https://blocklypro.webduino.com.cn');
          } else {
            return res.redirect('/');
          }
        });
